...
fun evaluateGuess(secret: String, guess: String): Evaluation {
    ...
    //combine secret and guess to one pair. then check values are same in each pairs.
    val rightPosition = secret.zip(guess).count { it.first == it.second }

    //ABCDEF: each colors value
    //sumBy: get sum result which each character(A,B,C,D,E,F)'s count in "secret" amd "guess".
    //If "secret" has 1 but "guess" has 0 about "A", it mean that "A" is not common letter.
    // If "secret" has 2 and "guess" has 3 about "B", it menat that "B" is common letter counted 2.
